...
contract AssetTracker is usingOraclize, Pausable {
    ...
    function registerAsset (uint256 _date, string _name, bytes32 _id) public {
        ...
        createEvent(msg.sender, _id, "Created", "CREATED", new bytes32[](0));
        assets[msg.sender].push(asset);
        emit AssetCreated(_id);
    }

    function createEvent(address _address, bytes32 _eventId, string _name, string _type, bytes32[] _data) internal {
    ...
    function fetchEvent(bytes32 _eventId) public view returns (string, string, bytes32[]) {
        AssetEvent assetEvent = assetEvents[msg.sender][_eventId];
        return(assetEvent.name, assetEvent.eventType, assetEvent.data);
    }
    ...
    function addEvent(bytes32 _assetId, bytes32 _eventId, string _name, string _type, bytes32[] _data) public {
        for (uint i = 0; i < assets[msg.sender].length; i++) {
            if (equal(assets[msg.sender][i].id, _assetId)) {
                assets[msg.sender][i].assetEventIds.push(_eventId);
                createEvent(msg.sender, _eventId, _name, _type, _data);
